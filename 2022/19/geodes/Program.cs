...
{
    public Dictionary<RobotType, (ResourceType, int)[]> RobotCosts;

    public Blueprint(Match match) {
        RobotCosts = new();

        RobotCosts.Add(RobotType.Ore, new[] { (ResourceType.Ore, int.Parse(match.Groups["OreRobotOreCost"].Value)) });

        RobotCosts.Add(RobotType.Clay, new[] { (ResourceType.Ore, int.Parse(match.Groups["ClayRobotOreCost"].Value)) });

        RobotCosts.Add(RobotType.Obsidian, new[] {
            (ResourceType.Ore, int.Parse(match.Groups["ObsidianRobotOreCost"].Value)),
            (ResourceType.Clay, int.Parse(match.Groups["ObsidianRobotClayCost"].Value))
        });

        RobotCosts.Add(RobotType.Geode, new[] {
            (ResourceType.Ore, int.Parse(match.Groups["GeodeRobotOreCost"].Value)),
            (ResourceType.Obsidian, int.Parse(match.Groups["GeodeRobotObsidianCost"].Value))
        });
    }
}
...
public class BluePrints
{
    ...
    public BluePrints(string blueprintsText) {

        Regex regex = new(@"Blueprint \d+: Each ore robot costs (?<OreRobotOreCost>\d+) ore. Each clay robot costs (?<ClayRobotOreCost>\d+) ore. Each obsidian robot costs (?<ObsidianRobotOreCost>\d+) ore and (?<ObsidianRobotClayCost>\d+) clay. Each geode robot costs (?<GeodeRobotOreCost>\d+) ore and (?<GeodeRobotObsidianCost>\d+) obsidian.");

        MatchCollection matches = regex.Matches(blueprintsText);

        Blueprints = new Blueprint[matches.Count];

        for (int k = 0; k < matches.Count; k++) {
            Blueprints[k] = new Blueprint(matches[k]);
        }
    }
}

public static class BlueprintCalculator
{
    public static int GetScore(Blueprint blueprint, int minutes) {
        return 0;
    }
}
...
Blueprint 24: Each ore robot costs 4 ore. Each clay robot costs 3 ore. Each obsidian robot costs 2 ore and 5 clay. Each geode robot costs 2 ore and 10 obsidian.
...
";
        BluePrints blueprints = new BluePrints(InputBluePrints);

        Console.WriteLine(BlueprintCalculator.GetScore(blueprints.Blueprints[0], 1));
    }
}
